...
import { Plus, Search, Edit, Trash2, ToggleLeft, ToggleRight, Package, Eye, AlertTriangle, Calculator, Info } from "lucide-react";
...
import { useKitsUsageCount } from "@/hooks/useProcedureKitsCRUD";
import { KitUsageBadge } from "./KitUsageBadge";
import type { MaterialKit, MaterialKitItem, MaterialCategory } from "@/types/cadastros-clinicos";
import { materialCategoryLabels, materialUnits } from "@/types/cadastros-clinicos";
import { usePermissions } from "@/hooks/usePermissions";

export function MaterialKitsTab() {
  const [search, setSearch] = useState("");
  const [kitDialogOpen, setKitDialogOpen] = useState(false);
  const [itemsDialogOpen, setItemsDialogOpen] = useState(false);
  const [addItemDialogOpen, setAddItemDialogOpen] = useState(false);
  const [editKit, setEditKit] = useState<MaterialKit | null>(null);
  const [selectedKit, setSelectedKit] = useState<MaterialKit | null>(null);
  const [confirmToggle, setConfirmToggle] = useState<MaterialKit | null>(null);
  const [deleteItemId, setDeleteItemId] = useState<string | null>(null);

  const { data: kits = [], isLoading } = useMaterialKitsList(true);
  const { data: materials = [] } = useMaterialsList();
  const { data: kitItems = [], isLoading: loadingItems } = useMaterialKitItems(selectedKit?.id || null);
  ...
  const handleViewItems = (kit: MaterialKit) => {
    setSelectedKit(kit);
  ...
  return (
    <div className="space-y-4">
            ...
            <Card key={kit.id} className={!kit.is_active ? "opacity-60" : ""}>
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <KitUsageBadge kitId={kit.id} compact />
              ...
              <CardContent>
                <div className="flex items-center justify-between text-sm text-muted-foreground mb-2">
                  <span>{kit.items_count || 0} materiais</span>
                  <TooltipProvider>
                    <Tooltip>
      ...
      <Dialog open={kitDialogOpen} onOpenChange={setKitDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editKit ? "Editar Kit" : "Novo Kit de Materiais"}</DialogTitle>
            <DialogDescription>
              Crie kits reutilizáveis para padronizar o uso de materiais
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input
                placeholder="Ex: Kit Curativo Simples"
      ...
      <Dialog open={itemsDialogOpen} onOpenChange={setItemsDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Gerencie os materiais que compõem este kit
            </DialogDescription>
          </DialogHeader>
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          {materialCategoryLabels[item.material_category as MaterialCategory] || item.material_category}
                        </Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {formatCurrency((item.material_unit_cost || 0) * item.quantity)}
                      </TableCell>
                      {canManage && (
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t">
              ...
              <span className="font-bold text-lg">
                {formatCurrency(kitItems.reduce((sum, item) => 
                  sum + (item.material_unit_cost || 0) * item.quantity, 0
                ))}
              </span>
            </div>
          </div>
      ...
      <Dialog open={addItemDialogOpen} onOpenChange={setAddItemDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Adicionar Material ao Kit</DialogTitle>
            <DialogDescription>
              Selecione o material e a quantidade
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) => updateItemField("material_id", value)}
              >
                <SelectTrigger>
      ...
      {/* Dialog de confirmação de toggle */}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmToggle?.is_active
                ? `Deseja desativar "${confirmToggle?.name}"?`
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleToggleStatus}>
              {toggleKitMutation.isPending ? "Salvando..." : "Confirmar"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={!!deleteItemId} onOpenChange={() => setDeleteItemId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Deseja remover este material do kit?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
